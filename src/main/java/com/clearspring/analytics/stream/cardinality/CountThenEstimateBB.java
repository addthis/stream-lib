 ...
 */
public class CountThenEstimateBB implements ICardinality, Externalizable
{
     ...
     */
    public CountThenEstimateBB(int tippingPoint, IBuilder<ICardinality> builder)
    {
     ...
     */
    public CountThenEstimateBB(byte[] bytes) throws IOException, ClassNotFoundException
    {
        ...
        if (tipped)
        {
            ...
            switch (type)
            {
                ...
                case HLPC:
                    ...
                    buff.put(bytes);
                    estimator = HyperLogLogPlusBB.Builder.build(buff);
                    break;
                case LLC:
                    estimator = new LinearCounting(bytes);
                    break;
    ...
    public void writeExternal(ObjectOutput out) throws IOException
    {
        ...
        if (tipped)
        {
            ...
            else if (estimator instanceof HyperLogLogPlusBB)
            {
                out.writeByte(HLPC);
            }
    ...
    public ICardinality merge(ICardinality... estimators) throws CardinalityMergeException
    {
        ...
        CountThenEstimateBB[] all = Arrays.copyOf(estimators, estimators.length + 1, CountThenEstimateBB[].class);
     ...
     *          if estimators are not mergeable (all must be CountThenEstimateBB made with the same builder)
     */
    public static CountThenEstimateBB mergeEstimators(CountThenEstimateBB... estimators) throws CardinalityMergeException
    {
        CountThenEstimateBB merged = null;
        int numEstimators = (estimators == null) ? 0 : estimators.length;
        if (numEstimators > 0)
        {
            List<ICardinality> tipped = new ArrayList<ICardinality>(numEstimators);
            List<CountThenEstimateBB> untipped = new ArrayList<CountThenEstimateBB>(numEstimators);

            for (CountThenEstimateBB estimator : estimators)
            {
                if (estimator.tipped)
                {
            ...
            if (untipped.size() > 0)
            {
                merged = new CountThenEstimateBB(untipped.get(0).tippingPoint, untipped.get(0).builder);

                for (CountThenEstimateBB cte : untipped)
                {
            ...
            {
                merged = new CountThenEstimateBB(0, new LinearCounting.Builder(1));
                merged.tip();
