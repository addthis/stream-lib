...
public class RegisterSetBB
{
    ...
    public RegisterSetBB(int count)
    {
        this(count, null);
    }

    public RegisterSetBB(int count, ByteBuffer initialValues)
    {
        this.count = count;
    ...
    public boolean updateIfGreater(int position, int value)
    {
        ...
        long curVal = this.M.getInt(bucket*4) & mask;
        long newVal = value << shift;
        if (curVal < newVal) {
            this.M.putInt(bucket*4,(int) ((this.M.getInt(bucket*4) & ~mask) | newVal));
            return true;
        } else {
            return false;
        }
    ...
    public void merge(RegisterSetBB that)
    {
        for (int bucket = 0; bucket < M.capacity()/4; bucket++)
        {
            ...
            for (int j = 0; j < LOG2_BITS_PER_WORD; j++)
            {
                ...
                int thisVal = (this.M.getInt(bucket*4) & mask);
                int thatVal = (that.M.getInt(bucket*4) & mask);

                word |= (thisVal < thatVal) ? thatVal : thisVal;
            }
            this.M.putInt(bucket*4, word);
        }
    }
